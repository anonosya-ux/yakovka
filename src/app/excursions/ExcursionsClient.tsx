...
export default function ExcursionsClient() {
  ...
  const openExcursionModal = (title: string) => {
    setSelectedExcursion(title);
    setModalOpen(true);
  };
  ...
  return (
    <>
      <CallbackModal 
        isOpen={modalOpen} 
        onClose={() => { setModalOpen(false); setSelectedExcursion(''); }} 
            ...
            {premiumExcursions.map((exc, idx) => (
              <div key={exc.id} className={`flex flex-col ${idx % 2 !== 0 ? 'md:flex-row-reverse' : 'md:flex-row'} gap-8 md:gap-16 items-center`}>
                ...
                <div className="w-full md:w-1/2 space-y-6">
                  ...
                  <button 
                    onClick={() => openExcursionModal(exc.title)}
                    className="mt-8 inline-block text-center bg-white text-stone-900 hover:bg-primary hover:text-white px-8 py-4 rounded-full font-bold transition-colors w-full md:w-auto text-lg cursor-pointer"
                  >
                    Оставить заявку
                  </button>
                </div>
        ...
        <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {additionalExcursions.map(item => (
              <div 
                key={item.id} 
                className="bg-white rounded-3xl p-6 border border-stone-100 shadow-sm hover:shadow-premium transition-shadow group cursor-pointer"
                onClick={() => openExcursionModal(item.title)}
              >
                ...
                <button 
                  className="mt-6 w-full text-center bg-stone-100 text-stone-700 hover:bg-primary hover:text-white px-6 py-3 rounded-xl font-bold transition-colors text-sm"
                >
                  Оставить заявку
                </button>
              </div>
