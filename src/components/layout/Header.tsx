  ...
  return (
    <>
      <header className={`fixed top-0 z-50 w-full transition-all duration-500 ${
        isScrolled 
          ? 'bg-white/95 backdrop-blur-xl shadow-[0_4px_30px_rgba(41,37,36,0.08)] border-b border-stone-200/30 py-0'
          : 'bg-gradient-to-b from-stone-950/80 to-transparent border-b border-transparent py-2'
      }`}>
        <div className="container mx-auto flex h-[72px] items-center justify-between px-6">
          <Link href="/" className="flex items-center gap-3 group">
            <YakovkaLogo className="w-[54px] h-[54px] group-hover:scale-105 transition-transform" />
            <span className={`font-heading font-bold text-xl tracking-tight hidden md:inline-block transition-colors ${
              isScrolled 
                ? 'bg-clip-text text-transparent bg-gradient-to-r from-stone-900 to-stone-600'
            ...
            <div className="hidden xl:flex flex-col items-end gap-0.5 mr-4">
              <a 
                ...
                className={`flex items-center gap-1.5 text-[15px] font-bold transition-colors ${
                  isScrolled ? 'text-stone-800 hover:text-primary' : 'text-white hover:text-primary drop-shadow-md'
                }`}
              >
              ...
              <a 
                ...
                className={`flex items-center gap-1.5 text-[12px] font-medium transition-colors ${
                  isScrolled ? 'text-stone-500 hover:text-primary' : 'text-white/70 hover:text-white'
                }`}
              >
                +7 (909) 097-52-09
              </a>
